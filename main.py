...
def download_youtube_audio(url: str, output_path: str) -> DownloadedSongInfo:
    ...
    """
    try:
        # 1. Check if output path is a folder, if not, create the folder
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        
        # 2. Download the best audio stream using pytube
        yt = YouTube(url)
        audio_stream: Stream = yt.streams.filter(only_audio=True).desc().first()
        output_file = audio_stream.download(output_path)
        
        # 3. Return the appropriate DownloadedSongInfo result
...
def separate_4stems(input_path: str, output_path: str, codec: Codec = Codec.MP3) -> SeparationInfo:
    ...
    """
    try:
        # 1. Check if input path/file exists, if not, just return None
        if not os.path.exists(input_path):
            print(f'Input path {input_path} does not exist.')
            return None
        
        # 2. If input exists, check if output_path exists, if not, create the folder
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        
        # 3. Separate the tracks using the Spleeter code and the supplied codec
        separator = Separator('spleeter:4stems')
        separator.separate_to_file(input_path, output_path, codec=codec)
        
        # 4. Return info if all is okay
